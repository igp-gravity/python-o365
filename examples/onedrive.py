# -*- coding: utf-8 -*-
"""
Created on Thu Jul 16 11:09:53 2020
...
client_id = '701c36fa-d363-4258-ba82-0e8a3d836ffa'  # Your client_id
client_secret = '*****'  # Your client_secret, create an (id, secret) at https://apps.dev.microsoft.com
tenant_id='bbcb9797-d502-4c33-801d-9c3272867d74'
scopes = ['basic', 'https://microsoftgraph.chinacloudapi.cn/Files.ReadWrite.All']
CHUNK_SIZE = 1024 * 1024 * 5


class O365Account():
    def __init__(self, client_id=client_id,
                 client_secret=client_secret, tenant_id= tenant_id,scopes=scopes):
    ...
    def authenticate(self):
        return self.account.authenticate()  ## Fix
...
if __name__ == '__main__':
    ...
    account = O365Account()
    #account.upload_file('D://seg.txt', 'ISPEC')
    #account.upload_file('D://mag3.txt', 'MICROG/2007')
    #account.download_file('ISPEC/seg.txt','E://')
    #account.upload_file('D://Qin2018_Article_DistributionOfIntra-CrustalLow.pdf', 'ISPEC')
    #account.upload_file('D://Geoist功能方向.pptx', 'ISPEC')
    #account.upload_file('D://地区震情概况模板.docx', 'ISPEC')
    
    folder = account.get_folder_from_path('ISPEC')
    items = folder.get_items()
    for item in items:
        if item.is_file:
            print(item.name)
